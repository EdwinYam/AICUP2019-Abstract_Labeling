...
export TRAIN_PATH=data/PaperAbstract/train_BACKGROUND.jsonl
export DEV_PATH=data/PaperAbstract/dev_BACKGROUND.jsonl
export TEST_PATH=data/PaperAbstract/test.jsonl
...
export BATCH_SIZE=4
export LR=5e-5
export TRAINING_DATA_INSTANCES=7000
export NUM_EPOCHS=3
...
CONFIG_FILE=abstract_labeling/config.jsonnet

# python -m allennlp.run predict --silent --cuda-device $cuda_device --predictor SeqClassificationPredictor --output-file PREDS.txt  --include-package sequential_sentence_classification $SERIALIZATION_DIR "$@" $TEST_PATH

python -m allennlp.run train $CONFIG_FILE  --include-package abstract_labeling -s $SERIALIZATION_DIR "$@"
